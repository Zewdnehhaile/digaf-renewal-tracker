...
export const notificationService = {
    ...
    showBrowserNotification: (title: string, body: string, icon?: string, onClick?: () => void): void => {
        ...
        try {
            // Check if the page is visible (not minimized or in background)
            if (document.hidden) {
                // Only show if page is hidden (background)
                const notification = new Notification(title, {
                    body: body,
                    icon: icon || '/favicon.ico',
                    silent: false,
                    requireInteraction: true,
                    tag: Date.now().toString(), // Prevent duplicate notifications
                });
                ...
                // Page is visible, show a less intrusive notification
                console.log(`📩 New message: ${title} - ${body}`);
                // Still show the notification even if page is visible (optional)
                const notification = new Notification(title, {
                    body: body,
                    icon: icon || '/favicon.ico',
                    silent: true,
